...
namespace cons {
	struct color_map_fast : color_map<wchar_t> {
		static const int charmap_count = 497;

		cpix table[charmap_count];

		void init() override {
			int clen = characters.size();
			...
			for (int i = 0, b = 0; i < 768; i++) {
				...
				table[b].ch = character;
				table[b++].co = foreground | background << 4;
			}
		...
		void getDitherColored(con_color r, con_color g, con_color b, wchar_t *ch, con_color *color) {
			...
			for (int i = 0; i < charmap_count; i++) {
				con_color cr = characterMap[i].r;
				con_color cg = characterMap[i].g;
				con_color cb = characterMap[i].b;
			...
			if (have_table) {
				*ch = table[nearest_index].ch;
				*color = table[nearest_index].co;
				return;
			} else {
				for (int i = 0, b = 0; i < 768; i++) {
					con_color background = ((i / 3) % 256) >> 4;
